...
function Topbar({ user, onLogout, onLogin, onRegister }: { user?: User | null; onLogout?: () => void; onLogin?: () => void; onRegister?: () => void }) {
  return <header className="topbar">
    <a className="brand" href="/" aria-label="WB الصفحة الرئيسية"><span className="brand-mark">و</span><span>WB</span></a>
...
function AuthScreen({ mode, onBack, onAuthenticated }: { mode: Exclude<AuthMode, 'landing'>; onBack: () => void; onAuthenticated: (user: User) => void }) {
  ...
  const copy = mode === 'register' ? ['أنشئ حسابك في WB', 'ابدأ من مجتمع عام، ثم اكتشف المساحات المغلقة التي تناسبك.'] : mode === 'login' ? ['مرحبًا بعودتك', 'أكمل من حيث توقفت.'] : mode === 'forgot' ? ['استعادة كلمة المرور', 'سنرسل رابطًا إذا كان الحساب موجودًا.'] : ['تفعيل البريد', 'ألصق رمز التفعيل الذي وصلك عبر البريد.'];
...
function LoadingScreen() { return <div className="app-shell"><Topbar /><main className="auth-wrap"><div className="empty">جارٍ تجهيز مساحة WB…</div></main></div>; }
